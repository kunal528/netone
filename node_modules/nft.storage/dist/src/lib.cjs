...
class NFTStorage {
  ...
  static async storeCar({ endpoint, token }, car, { onStoredChunk } = {}) {
    ...
    const splitter =
      car instanceof file.Blob
        ? await treewalk.TreewalkCarSplitter.fromBlob(car, targetSize)
        : new treewalk.TreewalkCarSplitter(car, targetSize);
    ...
    const upload = streamingIterables.transform(
      ...
      async function (/** @type {AsyncIterable<Uint8Array>} */ car) {
        ...
        const carFile = new file.Blob(carParts, {
   ...
   */
  static async store({ endpoint, token: token$1 }, metadata) {
    ...
    const response = await fetch__default['default'](url.toString(), {
      method: 'POST',
      headers: NFTStorage.auth(token$1),
      body,
    });
...
const validateERC1155 = ({ name, description, image, decimals }) => {
  ...
  if (!(image instanceof file.Blob) || !image.type.startsWith('image/')) {
    throw new TypeError(
